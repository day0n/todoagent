...
function AttemptRow({
  ...
  return (
    <div>
      ...
      >
        <span aria-hidden className="w-3 shrink-0 text-[0.625rem] text-[var(--color-fg-subtle)]">
          {open ? "▾" : "▸"}
        </span>
        ...
        <span className="ml-auto flex shrink-0 items-center gap-2.5">
          ...
          {failed ? <Badge tone="danger">{attempt.status}</Badge> : null}
        </span>
      </button>
      ...
      {open ? (
        <div
          ...
          style={{ borderColor: "var(--color-line)", background: "var(--color-surface-sunken)" }}
        >
          {loading ? (
            <Spinner label="加载成绩单" />
          ) : error !== null ? (
            ...
            <p className="meta">（无内容）</p>
          ) : (
            <>
              {transcript.error !== null ? (
                <p className="mb-2.5 text-[0.8125rem]" style={{ color: "var(--color-danger)" }}>
                  {transcript.error}
                </p>
              ) : null}
              {transcript.output !== null && transcript.output.length > 0 ? (
                <pre className="mono max-h-[24rem] overflow-auto whitespace-pre-wrap break-words text-[var(--color-fg-muted)]">
                  {transcript.output}
                </pre>
              ) : (
                <p className="meta">这次运行没有产生文本输出。</p>
              )}
            </>
          )}
        </div>
